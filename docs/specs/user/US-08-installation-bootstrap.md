...
# US-08 · User Spec — First-run installation + bootstrap

**Status:** Review
**Owner:** Chris
**Functional Spec:** [`functional/US-08-installation-bootstrap.md`](../functional/US-08-installation-bootstrap.md)
**Test Spec:** [`test/US-08-installation-bootstrap.md`](../test/US-08-installation-bootstrap.md)
**Sprint:** 2
**Depends on:** ADR-002 v1.1 (pip + CLI), ADR-003 (single admin seed, co-located default), US-07 (Ollama integration is what `init` probes).
...
#  → Writes default config to $COCKPIT_DATA_DIR/config.toml.
...
# 5. Log in as admin / ollama → forced password change (US-09) → use.
```
...
5. The seeded admin user (`admin` / `ollama`) is created with `must_change_password=true`. (Cross-validated by US-09.)
...
- The Bloxperts deployment uses `INITIAL_USERS` env var (when implemented in v0.2) to seed extra accounts. v0.1 only seeds `admin`; everyone else is created via US-06.
