<!-- Status: Review | Version: 0.3 | Created: 2026-04-27 | Updated: 2026-04-27 -->
# US-01 · User Spec — User logs in to the cockpit
...
**Sprint:** 2
**Related:** US-08 (installation seeds the first admin), US-09 (first-login forced password change), ADR-004 (role ladder).
...
If I am the seeded `admin` (US-08) or any user the admin created (US-06), I am required to change my password before I can do anything else (US-09).
...
11. If `users.must_change_password = 1` for the logged-in user, every protected route except `/api/auth/change-password` returns 409 with body `{"detail": "must_change_password"}`. The frontend renders a forced password-change screen. (Owned by US-09; cross-referenced here for completeness.)

## Scope boundaries (out)

- 2FA / TOTP — not in v0.1.
- OAuth, SAML, OIDC, single sign-on — not in v0.1.
- Self-service registration — not in v0.1 (admin creates users; US-06).
...
- Hard prerequisite for US-02, US-03, US-04, US-05, US-06, US-10 — none of those pages render without a session.
