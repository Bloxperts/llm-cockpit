...
# US-09 · Test Spec — First-login forced password change
...
| T-08 | AC-8 | After admin (US-06) `POST /api/admin/users/{id}/reset-password`, that user's `must_change_password` is back to `1`; next login goes through the flow. | auto |
