...
# US-10 · Functional Spec — Admin: Ollama configuration + metrics

**Status:** Review
**Depends on:** US-01 (login), US-07 (Ollama integration — `pull_model` and `delete_model` are on the same `LLMChat` port), ADR-004 (admin role + model tagging).
**Min role:** `admin`.
**User Spec:** [`../user/US-10-ollama-configuration.md`](../user/US-10-ollama-configuration.md)
**Test Spec:** [`../test/US-10-ollama-configuration.md`](../test/US-10-ollama-configuration.md)
**Bound DG:** DG-004 — extends US-07's `LLMChat` port with `pull_model` / `delete_model` write methods. Block at end of file.
...
|---|---|---|---|---|
| Ollama daemon | Read + write (pull / delete) | `LLMChat` (extended methods `pull_model`, `delete_model`) | `OllamaLLMChat` (US-07) | Outbound |
| `nvidia-smi` (optional) | Read (history) | `Telemetry` | `NvidiaSmiTelemetry` (US-02) | Outbound |
...
**Test seam:** `FakeLLMChat` from US-07 already supports `pull_model` and `delete_model`. Tests pin the side effects (model now appears, model now gone) by inspecting the fake's recorded calls.
