...
# US-09 · Functional Spec — First-login forced password change

**Status:** Review
**Depends on:** US-01 (login), US-08 (bootstrap creates the admin with `must_change_password=true`).
**User Spec:** [`../user/US-09-first-login-password-change.md`](../user/US-09-first-login-password-change.md)
...
- US-01: must use `current_user_must_be_settled` on every protected route except the two exceptions.
- US-06: every admin-created user lands with `must_change_password=true`. Reset password flips it back to `1`.
- US-08: bootstrap seeds the admin with `must_change_password=true`.
