...
# US-05 · Functional Spec — Code interface

**Status:** Review
**Depends on:** US-04 (shares backend machinery), US-07 (Ollama integration via `LLMChat` port), ADR-004 (role gate; code-tagged picker).
**Min role:** `code`.
**User Spec:** [`../user/US-05-code-page.md`](../user/US-05-code-page.md)
**Test Spec:** [`../test/US-05-code-page.md`](../test/US-05-code-page.md)
**Bound DG:** DG-004 — inherits the `LLMChat` port from US-07. No new boundary surface introduced.
...
- **Default system prompt:** read from the `settings` row whose key is `code_default_system_prompt` (admin-editable in US-10). Falls back to `"You are an expert pair programmer. Be terse, produce correct code, and prefer working examples over explanations."` if the row is absent.
...
Same `conversations` and `messages` tables as US-04. The `mode` column is set to `code` for conversations created from `/code`. The model picker stores last-used per `(user_id, mode)`.
...
- ✅ Switching models mid-conversation behaves the same as US-04.
