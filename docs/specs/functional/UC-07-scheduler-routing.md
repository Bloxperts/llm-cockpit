...
> **History note:** filename retained as `UC-07-scheduler-routing.md` to avoid a rename mid-sprint. The original SPEC-007 wired the cockpit through an AgenticBlox scheduler at port 8001; per ADR-003 §4 that dependency is dropped. v0.1 talks directly to Ollama.
...
- **Connection refused / DNS failure / 5 s connect timeout** → caller sees `OllamaUnreachableError`. Routers translate to HTTP 503. Dashboard badge flips per UC-02.
...
- `pull_model()` and `delete_model()` are admin-only actions (UC-10) but still go through the same port — same backend, same wire format, same fake.
