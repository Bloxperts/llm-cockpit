...
# UC-04 · User Spec — Chat interface

**Status:** Accepted
**Owner:** Chris
**Functional Spec:** [`functional/UC-04-chat-page.md`](../specs/functional/UC-04-chat-page.md)
**Test Spec:** [`test/UC-04-chat-page.md`](../specs/test/UC-04-chat-page.md)
**Sprint:** 4
**Depends on:** UC-01 (login), UC-07 (Ollama integration), ADR-004 (role gate; chat-tagged model picker).
**Min role:** `chat`.
...
- All chat traffic goes through the cockpit backend's `LLMChat` port, not directly to Ollama from the browser. (Owned by UC-07; mentioned here for completeness.)
...
- Same backend machinery as UC-05. The split is which models the picker shows and which page the user lands on.
- DG-004 inheritance from UC-07 (one outbound port: `LLMChat`). Functional spec records the inheritance.
