...
# UC-07 · User Spec — Ollama integration

**Status:** Accepted
**Owner:** Chris
**Functional Spec:** [`functional/UC-07-scheduler-routing.md`](../specs/functional/UC-07-scheduler-routing.md)
**Test Spec:** [`test/UC-07-scheduler-routing.md`](../specs/test/UC-07-scheduler-routing.md)
**Sprint:** 1 (architecture design) → 2 (implementation, in service of UC-08 + UC-01).
**Min role:** infrastructure (cross-cutting; not a user-facing screen).

> **Note on filename:** the file is named `UC-07-scheduler-routing.md` for v0.1 to avoid a rename mid-sprint. The story is **Ollama integration**, per ADR-003 §4 (scheduler dependency dropped).
...
- **`LLMChat`** — abstract interface in the cockpit core. Methods:
  ...
  - `pull_model(model) → AsyncIterator[PullProgress]` *(used by UC-10 only)*
  - `delete_model(model) → None` *(used by UC-10 only)*
...
- **Ollama unreachable.** Cockpit returns 503 to the chat / code routers. Dashboard flips to "Ollama unreachable" badge (UC-02).
...
- Pull-progress and delete (used by UC-10) are on the same port to avoid a second adapter for the same backend.
