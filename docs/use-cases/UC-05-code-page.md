...
# UC-05 · User Spec — Code interface

**Status:** Accepted
**Owner:** Chris
**Functional Spec:** [`functional/UC-05-code-page.md`](../specs/functional/UC-05-code-page.md)
**Test Spec:** [`test/UC-05-code-page.md`](../specs/test/UC-05-code-page.md)
**Sprint:** 4
**Depends on:** UC-01 (login), UC-04 (shares the backend chat machinery), UC-07 (Ollama integration), ADR-004 (role gate; code-tagged model picker).
**Min role:** `code`.
...
2. **Default system prompt** is a short coder system prompt — pre-filled on new conversations, editable. The default prompt lives in `config/code_default_system_prompt.md` and an admin can edit it on the Ollama config page (UC-10). The user can override per conversation.
...
7. Token-count footers and `messages.usage_in / usage_out` work identically to UC-04.
...
- Same backend machinery as UC-04; the split is the picker filter, default system prompt, and rendering tweaks.
- DG-004 inheritance from UC-07 (one outbound port: `LLMChat`). No new boundary surface.
