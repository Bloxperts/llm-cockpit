...
# UC-06 · User Spec — Admin: User management

**Status:** Accepted
**Owner:** Chris
**Functional Spec:** [`functional/UC-06-admin-controls.md`](../specs/functional/UC-06-admin-controls.md)
**Test Spec:** [`test/UC-06-admin-controls.md`](../specs/test/UC-06-admin-controls.md)
**Sprint:** 6
**Depends on:** UC-01 (login), UC-09 (must_change_password flow for newly-created users), ADR-004 (role ladder).
**Min role:** `admin`.

> **Note on filename:** the file is named `UC-06-admin-controls.md` for v0.1 to avoid a rename mid-sprint. The story is **user management**, per ADR-003 §6 (admin scope shrunk). The Ollama-config admin surface is a separate story (UC-10).
...
- **Add user button** → modal: `username`, `role` (default `chat`), and an option for "Set password now" (auto-generates a random password if unchecked, default checked = type a password). Created users land with `must_change_password=true` so they hit the UC-09 flow on first login.
- **Per-row actions:**
...
3. The new user logs in with the admin-set password, is forced through the UC-09 password change, then can use the cockpit.
...
5. Admin can reset any user's password. The user is forced through UC-09 on next login.
...
- All the original SPEC-006 content (pin/unpin model, num_ctx, keep_alive, power-cap, vLLM start/stop). Moved to **UC-10** (admin Ollama configuration) and v0.2.
