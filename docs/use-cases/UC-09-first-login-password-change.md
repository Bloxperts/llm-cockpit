...
# UC-09 · User Spec — First-login forced password change

**Status:** Accepted
**Owner:** Chris
**Functional Spec:** [`functional/UC-09-first-login-password-change.md`](../specs/functional/UC-09-first-login-password-change.md)
**Test Spec:** [`test/UC-09-first-login-password-change.md`](../specs/test/UC-09-first-login-password-change.md)
**Sprint:** 2
...
> As a user logging in for the first time — either the seeded `admin` (UC-08) or any user the admin created (UC-06) — I am required to change my password before I can do anything else, so that the default password (`ollama`) and admin-set passwords don't linger and become an attack surface.
...
- The change-password screen shows two fields: "New password", "Confirm new password". Submit:
  ...
  6. Redirects to the user's normal landing page (per UC-01 §AC-2).

The flow runs identically for the seeded admin and for admin-created users.

## Acceptance criteria

1. Seeded admin (UC-08) logs in with `admin` / `ollama`: backend authenticates and sets the JWT cookie, but the next request (e.g. `/api/auth/me`) returns 409 `{"detail": "must_change_password"}`.
...
5. After a successful change, the user is redirected to their role's landing page (UC-01 §AC-2).
...
8. An admin (UC-06) reset of any user's password sets `must_change_password=true` again; that user goes through the flow on next login.
...
- Account lockout after N failed change-password attempts. Out (login lockout is in UC-01; this flow is post-login).
...
- Cross-cuts UC-06: every admin-created user lands with `must_change_password=true`, so this flow is also the onboarding flow for new accounts.
