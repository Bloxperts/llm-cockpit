...
| `admin` | Above. Plus access `/admin/users` (UC-06) and `/admin/ollama` (UC-10). See system-wide metrics and audit logs. |
...
- **Admin-created users** (UC-06): admin picks the role at creation time. Default selection in the form is `chat` (least-privileged).
...
2. **Admin override** (UC-10). The Ollama-config admin page shows every model Ollama is serving with its current tag (`chat` / `code` / `both`). Admin can override per model. Override wins over heuristic. Override is persisted in `model_tags(model TEXT PRIMARY KEY, tag TEXT NOT NULL)`.
...
When an admin changes a user's role (UC-06), the next request that user makes — even with an existing valid JWT — reflects the new role. We achieve this by *not* baking the role into the JWT claims: the JWT only carries `sub` (user id). The role is resolved from the `users` table at every request via the `current_user` dependency. Cost is one indexed row read per request, which is negligible at five-user scale.
...
- DP-032 (privacy tiers) — chat history is private to the user; admin sees aggregate metrics, not message contents (UC-10 enforces this in the metrics view).
...
- UC-01 (login) emits a JWT carrying only `sub`; the `current_user` dependency resolves the role at request time.
- UC-06 (user management) supports setting role to any of the three.
- UC-10 (Ollama configuration) supports per-model tag override and exposes the heuristic config.
